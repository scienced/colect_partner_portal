...
import { ChevronLeft, ChevronRight, Play, FileText, Mail, ExternalLink, Download, BookOpen, Plus, RefreshCw } from "lucide-react"
...
export function ContentRow({ title, viewAllHref, items, variant = "default" }: ContentRowProps) {
...
function ContentCard({
  ...
  if (variant === "docs") {
    return (
      <a
        ...
        className={cn(
          "flex-shrink-0 group/card rounded-xl overflow-hidden bg-white border border-gray-200 hover:border-primary/30 hover:shadow-lg transition-all duration-300",
          cardWidth
        )}
      >
        <div className="p-4 flex gap-4">
          ...
          {item.external && <ExternalLink className="w-4 h-4 text-gray-400 flex-shrink-0" />}
        </div>
      </a>
    )
  }
  ...
  return (
    <CardWrapper
      {...cardProps}
      className={cn(
        "flex-shrink-0 group/card rounded-xl bg-white shadow-sm hover:shadow-xl transition-all duration-300 hover:scale-[1.02]",
        cardWidth
      )}
    >
      {/* Thumbnail */}
